 ...
 */
 ...
 data {
   ...
   vector<lower=0>[Nmeas] Aobs[2];                         // Prior observations for (heads,tails) of each A
   vector<lower=0>[Nmeas] Bobs[2];                         // Prior observations for (heads,tails) of each B
   matrix<lower=0>[Nmeas,Nstates] Cobs[2];                 // Prior observations for (heads,tails) of each C
 ...
 model {
   ...
   // prior on A and B
   A ~ beta(Aobs[1], Aobs[2]);
   B ~ beta(Bobs[1], Bobs[2]);

   for (idx_m in 1:Nmeas) {
     
     // jacobian transforms for A and B
     ...
     for (idx_s in 1:Nstates) {
       ...
       C[idx_m,idx_s] ~ beta(Cobs[1][idx_m,idx_s], Cobs[2][idx_m,idx_s]);
