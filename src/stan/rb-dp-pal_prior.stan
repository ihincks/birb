...
data {
  ...
  real <lower=0,upper=1> p_prior_p0;
  real <lower=0,upper=1> p_prior_z;
}
transformed data {
  real p_unc_est;
  real A_unc_est;
...
model {
  ...
  // PAL log prior, ignoring the constant terms
  if (p < p_prior_p0) {
    target += log(p) * (p_prior_p0 - p_prior_z) / ((1 - p_prior_p0) * p_prior_z);
  }
